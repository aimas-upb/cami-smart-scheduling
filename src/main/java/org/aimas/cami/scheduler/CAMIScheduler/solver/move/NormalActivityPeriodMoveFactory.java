...
public class NormalActivityPeriodMoveFactory implements MoveListFactory<ActivitySchedule> {

	@Override
	public List<? extends Move<ActivitySchedule>> createMoveList(ActivitySchedule solution) {
		List<NormalActivityPeriodMove> moveList = new ArrayList<>();
			...
			if (activity1 instanceof NormalActivity
				...
				for (Activity activity2 : activityList2) {
					if (activity2 instanceof NormalRelativeActivity
						...
						moveList.add(new NormalActivityPeriodMove(normalActivity,
								findPeriod(solution, normalActivity.getActivityPeriodTime(),
										normalRelativeActivity.getActivityPeriodWeekday().getDayIndex())));
					}
				}
